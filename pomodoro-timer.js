class PomodoroTimer {
  ...
  #startFocusTimer = async (cycle) => {
    ...
    cycle++
    if(this.settingsJson["cycle"] == 0 || cycle != this.settingsJson["cycle"]) {
  ...
  #printRemainingTime = (targetMinutes) => {
  ...
  #openAppAndWebpage = async (timerType) => {
    ...
    open(`page/html/${timerType}.html`, {app:{name:`${this.settingsJson["browser"]}`}});
    await this.#sleep(3000);
  
    if(this.settingsJson[`${timerType}App`]) await open.openApp(this.settingsJson[`${timerType}App`]);
    if(this.settingsJson[`${timerType}Url`]) await open(this.settingsJson[`${timerType}Url`], {app:{name:`${this.settingsJson["browser"]}`}});
    await this.#sleep(1000);
